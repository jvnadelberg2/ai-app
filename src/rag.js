...
function isText(p) {
  return /\.(md|txt|json|yaml|yml)$/i.test(p);
}
function chunk(s, size = 800, overlap = 200) {
  const out = [];
  ...
  for (const p of paths) {
    ...
    if (st.isDirectory()) {
      for (const f of fs.readdirSync(p)) {
        ...
        if (fs.statSync(fp).isFile() && isText(fp)) files.push(fp);
      }
    } else if (st.isFile() && isText(p)) {
      files.push(p);
    }
  }
  const db = load();
